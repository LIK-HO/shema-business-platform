...
def test_slo_contract_is_explicit_and_bounded() -> None:
    contract = json.loads((ROOT / "architecture" / "slo_contract.json").read_text(encoding="utf-8"))
    ...
    assert {item["id"] for item in contract["slis"]} == {"api_availability", "critical_mutation_success", "job_recovery", "outbox_lag", "external_effect_completion", "api_latency"}
